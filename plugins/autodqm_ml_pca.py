...
def comparators():
    return {"autodqm_ml_pca": autodqm_ml_pca}
...
def predict(hist, pca):
    ...
    sse = numpy.sum((hist - hist_reconstructed) ** 2).flatten()
...
def plot(histpair, hist, hist_reconstructed):
    ...
    bins = [
        (
            histpair.data_hist.axes[0].edges()[x]
            + histpair.data_hist.axes[0].edges()[x + 1]
        )
        / 2
        for x in range(0, len(histpair.data_hist.axes[0]))
    ]
    if bins[0] < -999:
    ...
    c.add_trace(
        go.Scatter(
            name="PCA Reco",
            x=bins,
            y=hist_reconstructed,
            marker_color="rgb(192, 255, 195)",
            opacity=0.7,
        )
    )
...
def autodqm_ml_pca(histpair, **kwargs):
    ...
    f_pca = "/var/www/cgi-bin/models/autodqm_ml_pca/{0}/{1}/{2}.json".format(
        histpair.config["jar_dir"], year, histpair.data_name
    )
    ...
    if (
        hist.shape[0] != pca.n_features_
    ):  # number of bins don't match (can happen if you try to apply a PCA trained on histograms from one year on another year, where that histogram has the same naming but its binning has been changed)
        show = False
        return None
    ...
    # Metadata
    artifacts = {"data": hist, "reco": hist_reconstructed}
    info = {
    ...
    return PluginResults(canvas, show=show, info=info, artifacts=artifacts)
