...
def StdDev(Data, Ref, func):
    ...
    if func == "Gaus1":
        ...
        output[mask] = (
            1.0 * nData * np.sqrt(np.clip(nRef - Ref, a_min=1, a_max=None))
        )[mask] / nRef
    elif func == "Gaus2":
        ...
        output = (
            ...
            * nData
            * np.sqrt(
                clipped / np.square(nRef)
                + Mean(nData, Ref, nRef, func) / np.square(nData)
            )
        )
        ...
        output[mask] = (
            ...
            * np.sqrt(
                clipped / np.square(nRef)
                + (nData - Mean(nData, Ref, nRef, func)) / np.square(nData)
            )
        )
