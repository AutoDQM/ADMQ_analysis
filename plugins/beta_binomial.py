...
def comparators():
    return {"beta_binomial": beta_binomial}
...
def beta_binomial(
    ...
    **kwargs,
    ...
    ref_hist_raw = numpy.round(numpy.copy(numpy.float64(ref_hist.values())))
    ...
    is_outlier = data_hist_Entries >= min_entries and (
        chi2 > chi2_cut or abs(max_pull) > pull_cut
    )

    ## plotting
    # Setting empty bins to be blank
    pull_hist = numpy.where(
        numpy.add(ref_hist_raw, data_hist_raw) == 0, None, pull_hist
    )
    ...
    if ("TH1" in str(type(data_hist))) and ("TH1" in str(type(ref_hist))):
        ...
        if data_hist_Entries > 0 and ref_hist_Entries > 0 and len(bins) > 15:
            last_bin = max(
                [
                    15,
                    max(numpy.nonzero(data_hist_raw)[0]),
                    max(numpy.nonzero(ref_hist_raw)[0]),
                ]
            )
        ...
        xAxisTitle = (
            xAxisTitle.replace("#circ", "\u00B0")
            .replace("#theta", "\u03B8")
            .replace("#phi", "\u03C6")
            .replace("#eta", "\u03B7")
        )
        yAxisTitle = (
            yAxisTitle.replace("#circ", "\u00B0")
            .replace("#theta", "\u03B8")
            .replace("#phi", "\u03C6")
            .replace("#eta", "\u03B7")
        )
        plotTitle = (
            plotTitle.replace("#circ", "\u00B0")
            .replace("#theta", "\u03B8")
            .replace("#phi", "\u03C6")
            .replace("#eta", "\u03B7")
        )
        ...
        c.add_trace(
            go.Bar(
                name="data:" + str(histpair.data_run),
                x=bins,
                y=data_hist_norm,
                marker_color="red",
            )
        ...
        c.update_layout(
            bargap=0, bargroupgap=0, barmode="overlay", plot_bgcolor="white"
        )
        c.update_xaxes(
    ...
    if ("TH2" in str(type(data_hist)) and "TH2" in str(type(ref_hist))) or (
        ...
        if data_hist.axes[0].labels():
            ...
            xLabels = [
                str(
                    data_hist.axes[0]._members["fXmin"]
                    + x
                    * (
                        data_hist.axes[0]._members["fXmax"]
                        - data_hist.axes[0]._members["fXmin"]
                    )
                    / data_hist.axes[0]._members["fNbins"]
                )
                for x in range(0, data_hist.axes[0]._members["fNbins"] + 1)
            ]
        ...
        if data_hist.axes[1].labels():
            ...
            yLabels = [
                str(
                    data_hist.axes[1]._members["fXmin"]
                    + x
                    * (
                        data_hist.axes[1]._members["fXmax"]
                        - data_hist.axes[1]._members["fXmin"]
                    )
                    / data_hist.axes[1]._members["fNbins"]
                )
                for x in range(0, data_hist.axes[1]._members["fNbins"] + 1)
            ]
    ...
    d = data_hist_raw
    r = ref_hist_raw
    with open("tmp/beta_binomial.csv", "a") as myfile:
        text = (
    ...
    return PluginResults(c, show=bool(is_outlier), info=info, artifacts=artifacts)
...
def StdDev(Data, Ref, func):
    ...
    if func == "Gaus1":
        ...
        output[mask] = (
            1.0 * nData * numpy.sqrt(numpy.clip(nRef - Ref, a_min=1, a_max=None))
        )[mask] / nRef
    elif func == "Gaus2":
        ...
        output = (
            ...
            * nData
            * numpy.sqrt(
                clipped / numpy.square(nRef)
                + Mean(nData, Ref, nRef, func) / numpy.square(nData)
            )
        )
        ...
        output[mask] = (
            ...
            * numpy.sqrt(
                clipped / numpy.square(nRef)
                + (nData - Mean(nData, Ref, nRef, func)) / numpy.square(nData)
            )
        )
    elif (func == "BetaB") or (func == "Gamma"):
        output = 1.0 * numpy.sqrt(
            nData
            * (Ref + 1)
            * (nRef - Ref + 1)
            * (nRef + 2 + nData)
            / (numpy.power(nRef + 2, 2) * (nRef + 3))
        )
...
def Prob(Data, nData, Ref, nRef, func, tol=0.01):
    ...
    if func == "Gamma":
        ...
        ab_ = nRef_tol + 2
        logProb = (
            gammaln(n_ + 1) + gammaln(k_ + a_) + gammaln(n_ - k_ + b_) + gammaln(ab_)
        )
        logProb -= (
            gammaln(k_ + 1)
            + gammaln(n_ - k_ + 1)
...
def ProbRel(Data, Ref, func, tol=0.01):
    ...
    exp_up = numpy.clip(
        numpy.ceil(Mean(Data, Ref, "Gaus1")), a_min=None, a_max=nData
    )  # make sure nothing goes above nData
    exp_down = numpy.clip(
        numpy.floor(Mean(Data, Ref, "Gaus1")), a_min=0, a_max=None
    ...
    ratio = numpy.divide(
        thisProb, maxProb, out=numpy.zeros_like(thisProb), where=maxProb != 0
    )
