...
def pullvals(
    ...
    **kwargs,
    ...
    if not (
        ("TH2" in str(type(data_hist)) and "TH2" in str(type(ref_hist)))
        or ("TProfile2D" in str(type(data_hist)) and "TProfile2" in str(type(ref_hist)))
    ):
        return None
    ...
    if norm_type == "row":
        ...
        data_hist_norm = normalize_rows(
            numpy.transpose(data_hist_norm), numpy.transpose(ref_hist_norm)
        )
    ...
    data_hist_errs = numpy.nan_to_num(
        abs(
            numpy.array(scipy.stats.chi2.interval(0.6827, 2 * data_hist_norm)) / 2
            - 1
            - data_hist_norm
        )
    )
    ref_hist_errs = numpy.nan_to_num(
        abs(
            numpy.array(scipy.stats.chi2.interval(0.6827, 2 * ref_hist_norm)) / 2
            - 1
            - ref_hist_norm
        )
    )
...
def pull(bin1, binerr1, bin2, binerr2):
    ...
    return numpy.divide(
        (bin1 - bin2),
        ((binerr1**2 + binerr2**2) ** 0.5),
        out=numpy.zeros_like(bin1),
        where=(binerr1 + binerr2) != 0,
    )
...
def normalize_rows(data_hist_norm, ref_hist_norm):
    ...
    sf = numpy.divide(
        ref_sum, data_sum, where=data_sum != 0, out=numpy.ones_like(data_sum)
    )
