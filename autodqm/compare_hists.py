    ...
    for hp in histpairs:
        ...
        for c in hp.comparators:
            try:
                ...
                raise error(
                    "Comparator {} was not found in {}/{}.".format(
                        c, dqmSource, subsystem
                    )
                )
...
def compile_histpairs(
    ...
    ref_file = [uproot.open(ref_path) for ref_path in ref_paths]
    ...
    for hconf in conf_list:
        ...
        if "*" not in h:
            if h in [str(keys)[0:-2] for keys in data_keys] and h in [
                str(keys)[0:-2] for keys in ref_keys
            ]:
                try:
            ...
            for name in data_keys:
                if (
                    h.split("*")[0] in str(name)
                    and name in ref_keys
                    and not "<" in str(name)
                ):
                    if "/" not in name[:-2]:
                        try:
    ...
    return histPairs[
        min(chunk_index, len(histPairs)) : min(chunk_index + chunk_size, len(histPairs))
    ]
...
def load_comparators(plugin_dir):
    ...
    for modname in os.listdir(plugin_dir):
        ...
        try:
            ...
            raise error(
                "Plugin {} from {} does not have a comparators() function.".format(
                    mod, plugin_dir
                )
            )
...
def identifier(hp, comparator_name):
    ...
    data_id = "DATA-{}-{}-{}".format(hp.data_series, hp.data_sample, hp.data_run)
    ...
    idname = "{}_{}_{}_{}_{}".format(data_id, ref_id, name_id, comp_id, hash_snippet)
    return idname
