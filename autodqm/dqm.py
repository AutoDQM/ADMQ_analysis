...
class DQMSession(FuturesSession):
    ...
    def stream_run(self, dqmSource, subsystem, series, sample, run, chunk_size=4096):
        ...
        if not os.path.exists(run_path):
            ...
            if isOnline:  ## Use cmsweb.cern.ch/dqm/offline/data/browse/ROOT/OnlineData/
                if not subsystem in OnlineMap.keys():
                    raise error(
                        "dqm.py stream_run: {} not in OnlineMap".format(subsystem)
                    )
                DQM_dir = OnlineMap[subsystem]
                run_info = next(
                    r
                    for r in runs
                    if r.name == run and DQM_dir + "_R000" in r.full_name
                )
        ...
        if VERBOSE:
            print("  * run_path = %s, size = %d" % (run_path, size))
    ...
    def fetch_series_list(self, dqmSource):
        ...
        if dqmSource == "Online":
            series_list = []
            for series in _resolve(
                self._fetch_dqm_rows(DQM_URL + "OnlineData/original/")
            ).data:
            ...
            raise error(
                "dqm.py fetch_series_list dqmSource = {}, not Onilne or Offline!".format(
                    dqmSource
                )
            )
    ...
    def fetch_sample_list(self, dqmSource, series):
        ...
        if VERBOSE:
            print(
                "\ndqm.py fetch_sample_list(dqmSource = %s, series = %s)"
                % (dqmSource, series)
            )
    ...
    def fetch_run_list(self, dqmSource, subsystem, series, sample, selRun=None):
        ...
        if selRun and len(str(selRun)) != 6:
            raise error(
                "dqm.py fetch_run_list selRun = {}, not 6 digits!".format(selRun)
            )
        ...
        for mr in macrorun_rows:
            if isOnline and mr.name != sample:
                continue
        ...
        for mr, fut in futures:
            ...
            for row in rows:
                if selRun and row.name != selRun:
                    continue
                if isOnline:
                    if not subsystem in OnlineMap.keys():
                        raise error(
                            "dqm.py fetch_run_list: {} not in OnlineMap".format(
                                subsystem
                            )
                        )
                    DQM_dir = OnlineMap[subsystem]
                    if not (DQM_dir + "_R000" in row.full_name):
                        continue
                run_rows.append(row)
            self._write_cache(mr, rows)

        if VERBOSE:
            print("  * len(run_rows) = %d" % len(run_rows))
        if VERBOSE + (len(run_rows) < 20) >= 2:
            print(run_rows)
        return run_rows
    ...
    def _stream_file(self, url, dest, chunk_size=4096):
        ...
        try:
            ...
            if cur != total:
                raise error(
                    "Failed to stream file: Final size {} less than total {}".format(
                        cur, total
                    )
                )
        except:
    ...
    def _run_path(self, dqmSource, subsystem, series, sample, run):
        ...
        if dqmSource == "Online":
            return "{}/{}.root".format(
                os.path.join(self.db, dqmSource, series, sample, OnlineMap[subsystem]),
                run,
            )
        elif dqmSource == "Offline":
            return "{}/{}.root".format(
                os.path.join(self.db, dqmSource, series, sample), run
            )
        else:
            raise error(
                "dqm.py _run_path dqmSource = {}, not Onilne or Offline!".format(
                    dqmSource
                )
            )
...
def _parse_run_full_name(full_name):
    ...
    if full_name.split("_")[2].startswith("R000"):  ## Format for OfflineData
        ...
        raise error(
            "dqm.py _parse_run_full_name({}), failed to parse run number!".format(
                full_name
            )
        )
        return "NULL"
